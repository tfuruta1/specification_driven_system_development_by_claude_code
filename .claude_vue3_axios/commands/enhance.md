...
## 概要
既存のVue.js + REST APIシステムに新機能を追加したり、既存機能を改善する際の設計と実装計画を立案します。既存アーキテクチャとの整合性を保ちながら、段階的な統合を実現します。
...
#### 2.1 バックエンド連携要件の確認
```javascript
// FastAPIバックエンドからのAPI仕様を取り込み
const importBackendAPISpec = async () => {
  const openApiPath = '../.claude_python_sqlAlckemy/.tmp/frontend_sync/openapi_spec.json'
  
  if (await fileExists(openApiPath)) {
    const openApiSpec = await readJSON(openApiPath)
    return parseOpenAPISpec(openApiSpec)
  }
...
// バックエンドAPIと新機能の統合
const integrateBackendAPIs = (backendSpec, newFeature) => {
  const integration = {
    feature: newFeature,
    backend_integration: {
      available_endpoints: [],
  ...
  // 利用可能なバックエンドAPIを確認
  if (backendSpec) {
    backendSpec.paths.forEach((path, methods) => {
      integration.backend_integration.available_endpoints.push({
        path,
        methods: Object.keys(methods),
        schemas: extractSchemas(methods)
      })
    })
  }
  
  // 新機能が必要とするAPIを特定
  const requiredAPIs = identifyRequiredAPIs(newFeature)
  requiredAPIs.forEach(api => {
    const exists = integration.backend_integration.available_endpoints.find(
      ep => ep.path === api.path && ep.methods.includes(api.method)
    )
    
    if (exists) {
      integration.backend_integration.required_endpoints.push(api)
    } else {
      integration.backend_integration.new_endpoints_needed.push(api)
    }
  })
...
// API Schema の更新
const generateApiSchemaUpdate = (changes) => {
  return `
...
```javascript
// バックエンドに新しいAPIが必要な場合
const requestBackendImplementation = async (newEndpoints) => {
  const backendRequest = {
    timestamp: new Date().toISOString(),
    source: 'vue3_frontend',
    feature_name: 'comment_system',
    required_endpoints: newEndpoints.map(ep => ({
      path: ep.path,
      method: ep.method,
  ...
  // バックエンドへの要求を保存
  await writeJSON('.tmp/api_sync/backend_requirements.json', backendRequest)
  
  console.log('バックエンド実装要求を生成しました:')
  console.log('cd ../claude_python_sqlAlckemy')
  console.log('/backend-sync spec_to_backend --source=".tmp/api_sync/backend_requirements.json"')
}
```
