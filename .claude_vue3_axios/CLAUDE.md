...
- **HTTP クライアント**: Axios (既存API連携強化)
- **API 連携**: REST API / GraphQL / SOAP / 既存システムAPI
- **認証**: JWT トークン / OAuth 2.0 / LDAP / 既存認証連携
- **ホスティング**: Vercel / Netlify / オンプレミス
- **バックエンド統合**: FastAPI + SQLAlchemy (完全統合対応 + venv連携)
- **既存システム統合**: レガシーシステム・DB・外部API統合
- **フルスタック**: venv + pip (統合開発) / Docker Compose (コンテナ版)
...
### REST API 連携ルール（既存システム統合強化）
- **サービス層の使用**: 直接的なAxios呼び出しを避け、サービス層経由で実行
- **既存システム適応**: レガシーAPI・SOAP・独自プロトコル対応
- **エラーハンドリング**: HTTPエラーレスポンスの適切な処理とユーザーフレンドリーなメッセージ
- **認証管理**: JWT トークン / LDAP / 既存認証システムの統合連携
- **データフォーマッティング**: 既存システムとのXML/JSON/CSVデータ変換
- **型安全性**: APIレスポンスの型定義を活用
- **リクエスト管理**: インターセプター活用による共通処理統一
- **venv連携**: FastAPIバックエンドとのシームレス統合開発
...
| 既存システム統合 | API設計 → フルスタック統合 → 認証設計 → エラーハンドリング |
| venv連携開発 | フルスタック統合ガイド → API設計 → 開発環境セットアップ |
...
- **データベーススペシャリスト**:
  - 専門領域: REST API向けDB設計・既存システム統合・レガシーDB連携・データ移行
  - 強み: MCPによる実DB連携、既存システムDB統合、複雑クエリ最適化、API設計、venv環境最適化
  - 主要活用: 既存DB統合設計、レガシーシステム連携、データ移行戦略、venv+FastAPI最適化、外部システム同期
...
#### 既存プロジェクト改善・システム統合時
1. **Gemini CLI**: 現状分析・改善戦略立案・既存システム評価・移行戦略策定
2. **Claude Code**: リファクタリング・機能追加・既存API統合・データフォーマット変換
3. **o3 MCP**: パフォーマンス改善・セキュリティ強化・レガシーDB統合・venv環境最適化
