# /field-transform - フィールド変換エンタープライズ専用コマンド

## 概要
複雑なデータ変換とフィールドマッピングに特化した包括的な変換コマンドです。ルールベース変換エンジン、100種類以上の変換関数、ビジュアルマッピング、変換履歴追跡を完全サポートします。
...
class RuleBasedTransformEngine:
    ...
    def _katakana_to_hiragana(self, text: str) -> str:
        """カタカナからひらがなへの変換"""
        return ''.join(
            chr(ord(ch) - 96) if 'ァ' <= ch <= 'ン' else ch
            for ch in text
        )
...
---
*このコマンドは複雑なフィールド変換とマッピングに特化しています。*