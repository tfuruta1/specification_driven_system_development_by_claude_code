# /signalr-realtime - SignalRリアルタイム通信専用コマンド

## 概要
SignalR Core（.NET Core 2.1+）とSignalR（.NET Framework）の両方に対応したリアルタイム双方向通信の実装コマンドです。WebSocket、Server-Sent Events、Long Pollingを自動的に切り替え、チャット、通知、ライブデータ更新、協調編集などのリアルタイム機能を提供します。
...
### 4. クライアント実装（JavaScript/TypeScript）
```typescript
...
## SignalR Legacy（.NET Framework）実装
...
| 機能 | SignalR Core (.NET 6+) | SignalR Core (.NET Core 3.1) | SignalR (.NET Framework) |
|------|------------------------|------------------------------|-------------------------|
| WebSockets | ✅ | ✅ | ✅ |
| Server-Sent Events | ✅ | ✅ | ✅ |
| Long Polling | ✅ | ✅ | ✅ |
...
| Strongly Typed Hubs | ✅ | ✅ | ❌ |
| Hub Filters | ✅ | ✅ | ❌ |
| Automatic Reconnect | ✅ Client-side | ✅ Client-side | ⚠️ Manual |
| Scale-out (Redis) | ✅ | ✅ | ✅ |
| Azure SignalR Service | ✅ | ✅ | ⚠️ Limited |
| Client Types | JS, .NET, Java, Python | JS, .NET, Java | JS, .NET |

## 出力レポート
```markdown
# SignalR リアルタイム通信レポート

## 実施項目
✅ Hub実装: 完了
✅ 認証・認可: 統合済み
✅ Redis backplane: 設定完了
✅ 自動再接続: 実装済み
✅ メッセージプロトコル: MessagePack対応

## パフォーマンス指標
- 同時接続数: 50,000+
- メッセージレイテンシ: <10ms
- スループット: 100,000 msg/s
- 再接続成功率: 99.5%

## スケーラビリティ
- Redis backplane: 水平スケール対応
- Azure SignalR Service: 無制限スケール
- Connection/User ratio: 1:5

## 推奨事項
1. Azure SignalR Service採用検討
2. MessagePack使用でペイロード削減
3. Connection管理の最適化
4. 監視・メトリクス実装
```
...
---
*このコマンドはSignalRを使用したリアルタイム通信の実装に特化しています。*