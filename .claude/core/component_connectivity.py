"""
コンポーネント接続テストモジュール - Claude Code Core
IntegrationTestRunnerから分離されたコンポーネント間の接続テスト機能
"""
...
class ComponentConnectivityTester:
    ...
    def get_connectivity_report(self) -> str:
        """接続テストレポート生成"""
        report = f"コンポーネント接続テスト結果:\n"
        report += f"登録されたコンポーネント数: {len(self.components)}\n"
        report += f"テストした接続数: {len(self.connections)}\n"
        report += f"検出された問題数: {len(self.connectivity_issues)}\n"
