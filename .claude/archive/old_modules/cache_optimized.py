...
class CacheOptimized:
    ...
    def __init__(self, cache_dir: Optional[str] = None):
        ...
        Args:
            cache_dir: キャッシュディレクトリパス（省略時は.claude/temp/cache）
        """
        self.cache_dir = Path(cache_dir) if cache_dir else paths.cache
...
class AnalysisCache(CacheOptimized):
    ...
    def __init__(self):
        # 相対パスを使用
        cache_analysis_dir = paths.cache / "core_analysis"
        super().__init__(cache_dir=str(cache_analysis_dir))
