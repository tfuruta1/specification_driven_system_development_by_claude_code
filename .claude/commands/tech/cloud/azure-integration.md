# /azure-integration - Azure統合最適化専用コマンド

## 概要
Microsoft Azureクラウドサービスとの統合に特化した包括的なコマンドです。Azure App Service、Functions、Storage、Cosmos DB、Service Bus、Application Insights等、主要なAzureサービスとの統合実装を提供します。
...
## 実施項目
✅ 認証統合: DefaultAzureCredential実装
✅ Storage統合: Blob Storage完全対応
✅ Cosmos DB: Change Feed対応
✅ Service Bus: メッセージング実装
✅ Functions: Durable Functions対応
✅ Application Insights: カスタムテレメトリ

## パフォーマンス指標
- 認証時間: <100ms (Managed Identity)
- Blob Upload: 100MB/s (並列処理)
- Cosmos DB RU: 最適化により40%削減
- Service Bus: 10,000 msg/s処理

## コスト最適化
- Storage: ライフサイクル管理で30%削減
- Cosmos DB: 適切なパーティション戦略で50%削減
- Functions: Consumption Planで従量課金

## 推奨事項
1. Managed Identity採用
2. Azure Monitor統合
3. Cost Management設定
4. Disaster Recovery計画
```
...
---
*このコマンドはMicrosoft Azureの主要サービスとの統合に特化しています。*