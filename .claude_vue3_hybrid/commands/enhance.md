...
## 概要
既存のVue.js + ハイブリッド接続システム（REST API → Supabase → JSONファイル）に新機能を追加したり、既存機能を改善する際の設計と実装計画を立案します。ハイブリッド接続アーキテクチャとの整合性を保ちながら、段階的な統合を実現します。
...
const loadExistingArchitecture = async () => {
  ...
  return {
    ...
    hybridConnection: await detectHybridConnectionConfig(),
    offlineStorage: await detectOfflineStoragePattern()
  }
}
